...
class VGG_IN(VGG_COSINE_SIMILARITY):
    def __init__(self, layer_index, instance_normalization_function=None, affine=False, pretrained=False, eps=torch.tensor(1e-08)):
        super(VGG_IN, self).__init__(pretrained=pretrained, eps=eps)
...
class VGG_COSINE(VGG_COSINE_SIMILARITY):
    def __init__(self, pretrained=False, eps=torch.tensor(1e-08)):
        super(VGG_COSINE, self).__init__(pretrained=pretrained, eps=eps)
...
class VGG_COSINE_ZEROS(torch.nn.Module):
    def __init__(self, pretrained=False):
        ...
        self.classifier = create_miniimagenet_classifier()
        self.layer_indices = SIMILARITY_layer_indices
...
#     checkpoint_path = pathJoin('trained_models', 'min', 'stylized_{}.ckpt'.format(model_type))
    ...
    if os.path.isfile(checkpoint_path):
        ...
        for dataset_name in dataset_names:
            eval_model_dataset(model, dataset_name, score_model)
